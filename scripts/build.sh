...
# Same flags as the proton-cachyos release CI x86_64_v3 build; the
# cachyos configure.sh turns CFLAGS/RUSTFLAGS into HOST_CFLAGS /
# HOST_RUSTFLAGS. Pre-set environment values win.
: "${CFLAGS:=-O3 -march=x86-64-v3 -mtune=core-avx2}"
: "${RUSTFLAGS:=-Copt-level=3 -Ctarget-cpu=nocona}"
export CFLAGS RUSTFLAGS

log "configuring (engine: $engine, build name: $BUILD_NAME)"
